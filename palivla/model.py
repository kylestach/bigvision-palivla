...
class PaliVLAModel(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        target_values = self.llm.compute_target_values(text_pre_logits, train=False)
        if isinstance(values, tuple):
            assert self.llm.model.value_head_type == "hlgauss"
            values, value_logits = values[0], values[1]
            info["value_logits"] = value_logits
            
            target_values, target_value_logits = target_values[0], target_values[1]
            info["target_value_logits"] = target_value_logits
...
def load_from_pretrained(
    ...
    *,
    ...
    def _init_params_fn(param_replacements):
        ...
        param_replacements["llm"]["target_value_head"] = params["llm"]["value_head"]
